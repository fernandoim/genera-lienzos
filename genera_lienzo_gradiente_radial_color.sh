...
function muestraayuda_genera_lienzo_gradiente_radial_color() {
  echo -e "\e[1mNOMBRE\e[0m"
  echo -e "\t genera_lienzo_franjas - Genera un lienzo con un degradado radial de dos colores aleatorios."
  echo ""
  ...
  echo -e "\t \e[1mgenera_lienzo_franjas\e[0m Genera un lienzo con un degradado radial de dos colores aleatorios."
...
function genera_lienzo_gradiente_radial_color() {

while [[ $# -gt 0 ]]
do
    case $1 in
        -h|ayuda)
            muestraayuda_genera_lienzo_gradiente_radial_color
            exit 0
            ;;
        -al|altura)
            let altura=$2
...
if [[ ! $ficherosalida ]]
then
  numeroficheros=$(ls lienzo_gradiente_radial_color* | wc -l)
  if [[ $numeroficheros = "0" ]]
  then
    ficherosalida="lienzo_gradiente_radial_color.png"
  else
    let numeral=$numeroficheros+1
    ficherosalida="lienzo_gradiente_radial_color"$numeral".png"
  fi
fi
...
instruccion="convert -size "$anchura"x"$altura" radial-gradient:"$color1"-"$color2" "$ficherosalida
eval $instruccion
...
genera_lienzo_gradiente_radial_color $*
