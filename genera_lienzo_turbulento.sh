...
function muestraayuda_genera_lienzo_turbulento() {
  echo -e "\e[1mNOMBRE\e[0m"
  echo -e "\t genera_lienzo_turbulento - Genera un lienzo con formas aleatorias de colores con turbulencias."
  echo ""
  echo -e "\e[1mSINOPSIS\e[0m"
  echo -e "\t genera_lienzo_turbulento.sh "
  echo -e "\t genera_lienzo_turbulento.sh -an 1920 -al 1080 nombredelfichero.png"

  echo ""
  echo -e "\e[1mDESCRIPCIÓN\e[0m"
  echo -e "\t \e[1mgenera_lienzo_turbulento\e[0m Genera un lienzo con formas aleatorias de colores con transparencia y líneas curvas aleatorias."
...
function genera_lienzo_turbulento() {

while [[ $# -gt 0 ]]
do
    case $1 in
        -h|ayuda)
            muestraayuda_genera_lienzo_turbulento
            exit 0
            ;;
        -al|altura)
            let altura=$2
...
if [[ ! $ficherosalida ]]
then
  numeroficheros=$(ls lienzo_turbulento* | wc -l)
  if [[ $numeroficheros = "0" ]]
  then
    ficherosalida="lienzo_turbulento.png"
  else
    let numeral=$numeroficheros+1
    ficherosalida="lienzo_turbulento_"$numeral".png"
  fi
fi
...
valores=$(genera_valores 6 0 20)

echo $valores


instruccion="gmic "$anchura","$altura",1,3 fx_seamless_turbulence "$valores" -output "$ficherosalida
eval $instruccion
echo $valores
