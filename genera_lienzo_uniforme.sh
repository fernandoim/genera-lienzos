...
function muestraayuda_genera_lienzo_uniforme() {
  echo -e "\e[1mNOMBRE\e[0m"
  echo -e "\t genera_lienzo_uniforme - Genera un lienzo con un color uniforme."
  echo ""
  echo -e "\e[1mSINOPSIS\e[0m"
  echo -e "\t genera_lienzo_uniforme.sh "
  echo -e "\t genera_lienzo_uniforme.sh -an 1920 -al 1080 nombredelfichero.png"

  echo ""
  echo -e "\e[1mDESCRIPCIÓN\e[0m"
  echo -e "\t \e[1mgenera_lienzo_uniforme\e[0m Genera un lienzo con un color uniforme."
...
function genera_lienzo_uniforme() {

while [[ $# -gt 0 ]]
do
    case $1 in
        -h|ayuda)
            muestraayuda_genera_lienzo_uniforme
            exit 0
            ;;
        -al|altura)
            let altura=$2
...
if [[ ! $ficherosalida ]]
then
  numeroficheros=$(ls lienzo_uniforme* | wc -l)
  if [[ $numeroficheros = "0" ]]
  then
    ficherosalida="lienzo_uniforme.png"
  else
    let numeral=$numeroficheros+1
    ficherosalida="lienzo_uniforme_"$numeral".png"
  fi
fi

# uniforme no requiere valores

color=$(convert -list color | cut -f 1 -d " " | shuf -n 1)


instruccion="convert -size "$anchura"x"$altura" canvas:"$color" "$ficherosalida
eval $instruccion
echo "Creando lienzo uniforme "$color
