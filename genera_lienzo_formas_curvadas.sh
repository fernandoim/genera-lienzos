...
function muestraayuda() {
  ...
  echo -e "\t genera_lienzo_formas_curvadas - Genera un lienzo con el dibujo de formas curvadas."
  echo ""
  echo -e "\e[1mSINOPSIS\e[0m"
  echo -e "\t genera_lienzo_formas_curvadas.sh "
  echo -e "\t genera_lienzo_formas_curvadas.sh -an 1920 -al 1080 nombredelfichero.png"

  echo ""
  echo -e "\e[1mDESCRIPCIÓN\e[0m"
  echo -e "\t \e[1mgenera_lienzo_formas_curvadas\e[0m Genera un lienzo con formas curvadas dibujadas."
  ...
  echo -e "\t [ ]\e[1m-b\e[0m | \e[1mblanco\e[0m ] Genera un lienzo blanco con el dibujo de un formas_curvadas en negro."
  echo -e "\t [ \e[1m-n\e[0m | \e[1mnegro\e[0m ]Genera un lienzo negro con el dibujo de un formas_curvadas en blanco."
  ...
  echo -e "\t \e[1mnombredelfichero.png\e[0m Indica el nombre del fichero que se generará con el lienzo. Si no se le indica un nombre de fichero, el valor por defecto es «lienzo_formas_curvadas.png»."

}

function genera_lienzo_formas_curvadas() {
  # Genera un lienzo con el dibujo de un formas_curvadas.
...
if [[ ! $ficherosalida ]]
then
  numeroficheros=$(ls lienzo_formas_curvadas* | wc -l)
  if [[ $numeroficheros = "0" ]]
  then
    ficherosalida="lienzo_formas_curvadas.png"
  else
    let numeral=$numeroficheros+1
    ficherosalida="lienzo_formas_curvadas_"$numeral".png"
  fi
fi



  # gmic maze $anchura_lienzo","$altura_lienzo negate normalize 0,255 -output formas_curvadas.png
  if [[ $negate -eq 1 ]]
  then
    gmic $anchura","$altura truchet , normalize 0,255 negate -output $ficherosalida
  else
    gmic $anchura","$altura truchet , normalize 0,255 -output $ficherosalida
  fi
