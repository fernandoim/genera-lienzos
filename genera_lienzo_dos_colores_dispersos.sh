...
function muestraayuda_genera_lienzo_dos_colores_dispersos() {
  echo -e "\e[1mNOMBRE\e[0m"
  echo -e "\t genera_lienzo_franjas - Genera un lienzo con dos colores aleatorios dispersos a lo largo del lienzo."
  echo ""
  ...
  echo -e "\t \e[1mgenera_lienzo_franjas\e[0m Genera un lienzo con dos colores aleatorios dispersos a lo largo del lienzo."
...
function genera_lienzo_dos_colores_dispersos() {

while [[ $# -gt 0 ]]
do
    case $1 in
        -h|ayuda)
            muestraayuda_genera_lienzo_dos_colores_dispersos
            exit 0
            ;;
        -al|altura)
            let altura=$2
...
if [[ ! $ficherosalida ]]
then
  numeroficheros=$(ls lienzo_dos_colores_dispersos* | wc -l)
  if [[ $numeroficheros = "0" ]]
  then
    ficherosalida="lienzo_dos_colores_dispersos.png"
  else
    let numeral=$numeroficheros+1
    ficherosalida="lienzo_dos_colores_dispersos"$numeral".png"
  fi
fi
...
instruccion="convert -size "$anchura"x"$mediaaltura" xc:"$color1" xc:"$color2" -append -spread "$valordispersion" "$ficherosalida
eval $instruccion
...
genera_lienzo_dos_colores_dispersos $*
