...
function muestraayuda_genera_lienzo_franjas_verticales() {
  echo -e "\e[1mNOMBRE\e[0m"
  echo -e "\t genera_lienzo_franjas - Genera un lienzo con franjas verticales blancas y negras de un tamaño aleatorio."
  echo ""
  ...
  echo -e "\t \e[1mgenera_lienzo_franjas\e[0m Genera un lienzo con franjas verticales blancas y negras de un tamaño aleatorio."
...
function genera_lienzo_franjas_verticales() {
...
if [[ ! $ficherosalida ]]
then
  numeroficheros=$(ls lienzo_franjas* | wc -l)
  if [[ $numeroficheros = "0" ]]
  then
    ficherosalida="lienzo_franjas.png"
  else
    let numeral=$numeroficheros+1
    ficherosalida="lienzo_franjas_"$numeral".png"
  fi
fi
...
instruccion="convert -size "$grosor"x"$altura" "$franjas" +append -extent "$anchura"x"$altura" "$ficherosalida
eval $instruccion
...
genera_lienzo_franjas_verticales $*
