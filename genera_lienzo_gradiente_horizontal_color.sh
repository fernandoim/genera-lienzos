...
function muestraayuda_genera_lienzo_gradiente_horizontal_color() {
  ...
  echo -e "\t genera_lienzo_franjas - Genera un lienzo con un degradado horizontal de dos colores aleatorios."
  echo ""
  ...
  echo -e "\t \e[1mgenera_lienzo_franjas\e[0m Genera un lienzo con un degradado horizontal de dos colores aleatorios."
...
function genera_lienzo_gradiente_horizontal_color() {
...
instruccion="convert -size "$altura"x"$anchura" gradient:"$color1"-"$color2" -rotate 90 "$ficherosalida
eval $instruccion
...
genera_lienzo_gradiente_horizontal_color $*
