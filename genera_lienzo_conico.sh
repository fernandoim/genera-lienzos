...
function muestraayuda_genera_lienzo_conico() {
  echo -e "\e[1mNOMBRE\e[0m"
  echo -e "\t genera_lienzo_franjas - Genera un lienzo con un efecto cónico de dos colores aleatorios."
  echo ""
  ...
  echo -e "\t \e[1mgenera_lienzo_franjas\e[0m Genera un lienzo con un efecto cónico de dos colores aleatorios."
...
function genera_lienzo_conico() {

while [[ $# -gt 0 ]]
do
    case $1 in
        -h|ayuda)
            muestraayuda_genera_lienzo_conico
            exit 0
            ;;
        -al|altura)
            let altura=$2
...
if [[ ! $ficherosalida ]]
then
  numeroficheros=$(ls lienzo_conico* | wc -l)
  if [[ $numeroficheros = "0" ]]
  then
    ficherosalida="lienzo_conico.png"
  else
    let numeral=$numeroficheros+1
    ficherosalida="lienzo_conico"$numeral".png"
  fi
fi
...
instruccion="convert -size "$anchura"x"$mediaaltura" xc:"$color1" xc:"$color2" -append -radial-blur 225 "$ficherosalida
eval $instruccion
