...
function muestraayuda_genera_lienzo_plasma() {
  echo -e "\e[1mNOMBRE\e[0m"
  echo -e "\t genera_lienzo_plasma - Genera un lienzo con formas aleatorias de colores con turbulencias."
  echo ""
  echo -e "\e[1mSINOPSIS\e[0m"
  echo -e "\t genera_lienzo_plasma.sh "
  echo -e "\t genera_lienzo_plasma.sh -an 1920 -al 1080 nombredelfichero.png"

  echo ""
  echo -e "\e[1mDESCRIPCIÓN\e[0m"
  echo -e "\t \e[1mgenera_lienzo_plasma\e[0m Genera un lienzo con formas aleatorias de colores con transparencia y líneas curvas aleatorias."
...
function genera_lienzo_plasma() {

while [[ $# -gt 0 ]]
do
    case $1 in
        -h|ayuda)
            muestraayuda_genera_lienzo_plasma
            exit 0
            ;;
        -al|altura)
            let altura=$2
...
if [[ ! $ficherosalida ]]
then
  numeroficheros=$(ls lienzo_plasma* | wc -l)
  if [[ $numeroficheros = "0" ]]
  then
    ficherosalida="lienzo_plasma.png"
  else
    let numeral=$numeroficheros+1
    ficherosalida="lienzo_plasma_"$numeral".png"
  fi
fi

# Plasma no requiere valores

instruccion="gmic "$anchura","$altura",1,3 plasma -output "$ficherosalida
eval $instruccion
echo $valores
