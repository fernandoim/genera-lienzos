...
function muestraayuda_genera_lienzo_franjas_angulares() {
  echo -e "\e[1mNOMBRE\e[0m"
  echo -e "\t genera_lienzo_franjas - Genera un lienzo con franjas angulares blancas y negras de un tamaño aleatorio."
  echo ""
  ...
  echo -e "\t \e[1mgenera_lienzo_franjas\e[0m Genera un lienzo con franjas angulares blancas y negras de un tamaño aleatorio."
...
function genera_lienzo_franjas_angulares() {

while [[ $# -gt 0 ]]
do
    case $1 in
        -h|ayuda)
            muestraayuda_genera_lienzo_franjas_angulares
            exit 0
            ;;
        -al|altura)
            let altura=$2
...
if [[ ! $ficherosalida ]]
then
  numeroficheros=$(ls lienzo_franjas_angulares* | wc -l)
  if [[ $numeroficheros = "0" ]]
  then
    ficherosalida="lienzo_franjas_angulares.png"
  else
    let numeral=$numeroficheros+1
    ficherosalida="lienzo_franjas_angulares"$numeral".png"
  fi
fi

divisor=$(shuf -i 50-100 -n 1)

if [[ $anchura -gt $altura ]]
then
  let grosor=$anchura/$divisor
  let rondas=$anchura/$grosor
else
  let grosor=$altura/$divisor
...
do
  resto=$(expr $i % 2)
    ...
    if [ $resto -eq 0 ]
    then
      bordes=$bordes"-bordercolor White -border "$grosor"x"$grosor" "
    else
      bordes=$bordes"-bordercolor Black -border "$grosor"x"$grosor" "
    fi
done

gravedad=$(convert -list gravity | shuf -n 1)

# -gravity "$gravedad"

instruccion="convert -size "$grosor"x"$grosor" xc:Black "$bordes" -gravity "$gravedad" -extent "$anchura"x"$altura" "$ficherosalida
eval $instruccion
...
genera_lienzo_franjas_angulares $*
